        ...
        if (req.method === "POST") {
          ...
          try {
            if (body.mode === "saveProfile") {
              ...
              const profileId = typeof body.profileId === "string" ? body.profileId : "";
              const currentActive = s.get(SETTINGS_NS)?.activeProfile ?? "default";
              if (profileId === "" || profileId === currentActive) {
                json(res, 400, { ok: false, error: "cannot delete the active profile" });
                return;
              }
