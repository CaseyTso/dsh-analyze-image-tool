...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return el(
        ...
        el(
          ...
          el(
            ...
            el("span", { style: { fontWeight: 600, fontSize: 13 } }, "analyze-image-tool"),
            el("span", { style: { opacity: 0.65, fontSize: 11 } }, "识图插件：视觉端点、模型、调用参数与粘贴图片模板"),
          ),
          el("span", { "aria-hidden": "true", style: { opacity: 0.7 } }, open ? "▾" : "▸"),
        ),
        open
          ? el(
              "div",
              { style: { padding: "4px 12px 12px", borderTop: "1px solid var(--dsh-color-border, #80808040)" } },
              el(SettingsPanel, { embedded: true, hideTitle: true }),
            )
          : null,
      );
    }
    ...
    function apply(ctx) {
      ...
      // Official rc7 plugin-settings slot: the Plugins section's 可配置 tab
      // dispatches `settings.plugin.item` by the settings namespace a plugin
      // registered on the host. Our host half registers `analyze-image-tool`,
      // so keying the card by that namespace makes the panel appear exactly
      // where the new web UI lists configurable plugins.
      ctx.slots.inject("settings.plugin.item", () => {
        const slotKind = ctx.slots.spec?.("settings.plugin.item")?.kind;
        return ctx.slots.register({
          name: "settings.plugin.item",
          // rc7 declares this slot as kind "keyed" and looks entries up by
          // the served namespace; older dsh releases declared it as a plain
          // list, which requires an id instead.
          ...(slotKind === "keyed" ? { key: SETTINGS_NS } : { id: "analyze-image-tool" }),
        }, SettingsPluginCard);
      });
    }

    exports.EyeDockEntry = EyeDockEntry;
    exports.SettingsPluginCard = SettingsPluginCard;
    exports.apply = apply;
